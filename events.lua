function register_events(event)
    ...
    for _, player in pairs(game.players) do
        stats = {{player.force.item_production_statistics, gauge_item_production_input, gauge_item_production_output},
                 ...
                 {player.force.entity_build_count_statistics, gauge_entity_build_count_input,
        ...
        evolution = {{player.force.evolution_factor, "total"},
        ...
        for name, n in pairs(player.force.logistic_networks) do
            for i in ipairs(n) do
                gauge_logistic_network_all_logistic_robots:set(n[i].all_logistic_robots,
                    {player.force.name, name, tostring(i)})
                gauge_logistic_network_available_logistic_robots:set(n[i].available_logistic_robots,
                    {player.force.name, name, tostring(i)})
                gauge_logistic_network_all_construction_robots:set(n[i].all_construction_robots,
                    {player.force.name, name, tostring(i)})
                gauge_logistic_network_available_construction_robots:set(n[i].available_construction_robots,
                    {player.force.name, name, tostring(i)})
    ...
    if server_save then
        game.write_file("game.prom", prometheus.collect(), false, 0)
    else
        game.write_file("game.prom", prometheus.collect(), false)
    end
end
